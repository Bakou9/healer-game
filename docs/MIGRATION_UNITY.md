...
La version **Phaser** a été retirée du dépôt (D-045) ; elle reste consultable dans l'historique Git
à l'étiquette `phaser-archive` (`git show phaser-archive:<chemin>`). Dernier état de Phaser repris : `91d7beb`.
Les chemins Phaser ci-dessous se lisent donc **à cette étiquette**, pas dans l'arborescence actuelle.
