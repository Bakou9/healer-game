# Cycle complet de vÃ©rification de la version Unity : construit l'exÃ©cutable Windows, le lance en mode
# capture (le bot de rÃ©fÃ©rence joue en accÃ©lÃ©rÃ©) et Ã©crit les captures d'Ã©cran dans %TEMP%\healer-captures.
...
if (-not $SkipBuild) {
  ...
  if (-not $p.WaitForExit(400000)) { try { Stop-Process -Id $p.Id -Force } catch {}; Write-Output "BUILD : dÃ©lai dÃ©passÃ©" }
  Write-Output ("BUILD : code {0} en {1:N0} s" -f $p.ExitCode, $sw.Elapsed.TotalSeconds)
  $t = Get-Content $log -ErrorAction SilentlyContinue
  $errs = $t | Select-String -Pattern "error CS\d+|\[Healer\] Ã‰CHEC|Exception:" | Select-Object -First 8
...
$g = Start-Process -FilePath $exe -ArgumentList @("-screen-width", "1280", "-screen-height", "720", "-screen-fullscreen", "0", "-healer-shots", $Shots, "-healer-out", $out, "-healer-speed", "6") -PassThru
if (-not $g.WaitForExit(150000)) { try { Stop-Process -Id $g.Id -Force } catch {}; Write-Output "JEU : dÃ©lai dÃ©passÃ©" }
