...
UNITY_FBX = os.path.normpath(os.path.join(HERE, "..", "..", "unity", "HealerGame", "Assets", "Resources", "Parts", "DruidMcp.fbx"))
...
# =============================================================================================
robe = lathe("Root__Robe", ROBE, [(0.72, 0.05), (0.60, 0.45), (0.47, 0.95), (0.36, 1.35), (0.30, 1.55)], sides=16, zigzag=0.10, jitter=0.03, folds=9, fold_depth=0.07)
add("Root", robe)
hem = lathe("Root__Hem", ROBE_DARK, [(0.74, 0.03), (0.73, 0.13)], sides=16, zigzag=0.06, jitter=0.03, cap_top=False, folds=9, fold_depth=0.07)
add("Root", hem)
...
# croissant de lune en bois autour du cristal (ouvert vers le haut)
cc = (bx, by, 2.30)
arc = [(cc[0] + 0.24 * math.cos(math.radians(t)), cc[1], cc[2] + 0.24 * math.sin(math.radians(t))) for t in range(-70, 251, 40)]
add("Weapon", tube("Weapon__Crescent", BARK, arc, [0.012, 0.03, 0.038, 0.04, 0.038, 0.03, 0.022, 0.012, 0.008], sides=5))
add("Weapon", tube("Weapon__Crescent", BARK, [(bx, by, 2.0), (bx, by, 2.1), (cc[0], cc[1], 2.06)], [0.04, 0.03, 0.02], sides=5))
...
print("DRUIDE MCP TRIANGLES", tris)

lib.setup_render(560, 860)
lib.render_views(os.path.join(OUT, "druid_mcp"), center=(0, 0, 1.6), distance=9.0, lens=75)

os.makedirs(os.path.dirname(UNITY_FBX), exist_ok=True)
lib.export_fbx(UNITY_FBX)
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT, "druid_mcp.blend"))
